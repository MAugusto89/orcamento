  ...
  const atualizarUsuario = async (
    ...
    try {
      a = await db.runAsync(
        ...
        [novoUsuarioNome, novoUsuarioEmail, novoUsuarioTelefone, usuarioId]
      );
      Alert.alert("Atenção!", "Usuário salvo com sucesso!");
      await getUsuarios();
    } catch (error) {
...
export const adicionarEmpresa = async (novaEmpresa) => {
  try {
    const query = await db.prepareAsync(
      "INSERT INTO usuario (nome, cnpj, email, telefone) VALUES (?, ?, ?, ?)"
    );
